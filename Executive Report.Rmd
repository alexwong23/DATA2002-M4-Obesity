...
The results of this model is visualised by Figure 3.5 through the sigmoid function. The predicted values to probabilities are mapped between 0 and 1. For predictions of 0.5 and above, these are classifed as people who are overweight. Whereas predictions of below 0.5 are classified as people who are non-overweight.
...
plot_model(glm12, type = "pred", terms = c("abdomen", "chest", "thigh"), show.data = TRUE) + theme_bw(base_size = 9) + labs(title = 'Figure 3.5: Predicted value of overweight', caption='Source: SOCR Data')
```
...
plot(as.party(ov_tree), main="Figure 3.6 Decision Tree on Abdomen and Chest variables")
```