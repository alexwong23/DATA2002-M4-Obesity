...
Notably, waist, chest, abdomen are highly correlated, and this may be due to the fact that they are from a similar body area. Hence, body measurements that are from similar areas are classified and linked together using the above graph.
...
All three QQ plots had shown a straight lines and this satisfies the normality assumption. However, the residual plots showed a slight variation for all three indicators, but given that the residual units were quite small, it is acceptable for the current analysis (Appendix A). 

Overall, abdomen is relatively the most important measurement in predicting all three obesity indicator and this result was expected as it corresponds to the previous correlation matrix analysis (Appendix B). 
...
plot_model(glm12, type = "pred", terms = c("abdomen", "chest", "thigh"), show.data = TRUE) + theme_bw(base_size = 9) + labs(caption='Source: SOCR Data',title = "Figure 2: Predicted values of Overweight")
```
...
M7<- lm(formula = density ~ neck + abdomen + hip + thigh , 
    data = data_density)
par(mfrow=c(3,1))
autoplot(bf_step_back,which=1:2)+theme_bw()
autoplot(bmi_step_back,which=1:2)+theme_bw()
